    ...
    try {
      ...
      if (removedSkills.length > 0) {
        const { error: deleteError } = await supabase
          ...
          .delete()
          .eq("person_id", user.id)
          .in("skill_id", removedSkills.map((s) => s.skill_id))
    ...
    <main className="relative flex min-h-dvh items-center justify-center bg-[#e8e9e8] px-5 py-10 text-[#22393c] sm:px-8">
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="relative w-full max-w-md">
        ...
        <div className="mt-8 flex items-center justify-between">
          {step > 1 ? (
            <button onClick={() => setStep(step - 1)} className="flex items-center gap-2 text-sm font-semibold text-[#668184] hover:text-[#22393c]">
