...
export function ProfileView() {
  ...
  const { projects, refetch: refetchProjects } = useUserProjects(user?.id, 5)
    ...
    try {
      ...
      if (skillsData) {
        const skillNames = skillsData
          ...
          .filter(Boolean)
        setSkillIds(skillsData.map((s: any) => s.skill_id).filter(Boolean))
  ...
  if (loading || !user) {
    return <div className="flex min-h-dvh items-center justify-center bg-[#e8e9e8] dark:bg-[#16241f]"><Loader2 className="size-10 animate-spin text-[#668184]" /></div>
  }
  ...
  return (
    <main className="relative min-h-dvh bg-[#e8e9e8] dark:bg-[#16241f] px-5 pb-32 pt-10 text-[#22393c] dark:text-[#f4f1ea] sm:px-8 transition-colors duration-300">
      ...
      <div className="relative mx-auto w-full max-w-md">
        ...
        <motion.section initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="mb-6">
          <div className="mb-3 flex items-center justify-between">
            ...
            <span className="text-xs font-semibold uppercase tracking-wider text-[#8a9a7b]">{connections.length} total</span>
          </div>
      ...
      {isEditing && (
        <EditProfileModal onClose={() => setIsEditing(false)} onSaved={handleProfileSaved} currentProfile={profile} currentSkills={skillIds} />
      )}
    </main>
  )
}
