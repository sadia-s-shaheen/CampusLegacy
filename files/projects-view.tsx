...
const filterOptions = ["All", "Active", "Legacy", "Completed"]
        ...
        <motion.header initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="mb-6 flex items-center justify-between">
          ...
          <div className="relative">
            <button
              ...
              className="glass-button glass-neutral flex h-9 w-9 items-center justify-center rounded-full"
            >
            ...
            <AnimatePresence>
              {isFilterOpen && (
                <motion.div
                  ...
                  exit={{ opacity: 0, y: -8 }}
                  className="glass-button glass-neutral absolute right-0 top-11 z-10 w-40 rounded-2xl p-1.5 shadow-lg"
                >
                  {filterOptions.map((opt) => (
                    <button
                      key={opt}
                      onClick={() => { setStatusFilter(opt); setIsFilterOpen(false) }}
                      className="flex w-full items-center justify-between rounded-xl px-3 py-2 text-left text-xs font-semibold text-[#22393c] hover:bg-[#22393c]/5"
                    >
                      {opt}
                      {statusFilter === opt && <Check className="size-3.5" />}
                    </button>
                  ))}
                </motion.div>
              )}
              ...
              return (
                <Link href={`/projects/${project.id}`} key={project.id}>
                  ...
                  >
                      ...
                      <span
                        className={`rounded-full px-2.5 py-0.5 text-[10px] font-bold uppercase tracking-wider ${
                          project.status === "active"
                            ? "bg-[#8a9a7b]/20 text-[#22393c]"
                            : project.status === "legacy"
                            ? "bg-[#22393c]/10 text-[#22393c]"
                    ...
                    <p className="mb-3 text-xs leading-relaxed text-[#22393c]/80 line-clamp-2">{project.description}</p>
