          ...
          <motion.section initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
            ...
            {isOwner && (
              <div className="mb-8 flex flex-col sm:flex-row gap-3">
                <button onClick={() => setShowAddRole(true)} className="flex flex-1 items-center justify-center gap-2 rounded-2xl bg-[#8a9a7b] py-3 text-sm font-semibold text-white transition-transform hover:scale-[1.02]">
                  <Plus className="size-4" /> Create Open Role
                </button>
                <button onClick={() => setShowInvite(true)} className="flex flex-1 items-center justify-center gap-2 rounded-2xl bg-[#22393c] py-3 text-sm font-semibold text-white transition-transform hover:scale-[1.02]">
                ...
                <div className="space-y-3">
                  {openRoles.map((role: any) => (
                    <div key={role.id} className="glass-button glass-lilac rounded-2xl p-5">
                      ...
                      {!isOwner && (
                        <button onClick={() => setApplyRole(role)} className="w-full rounded-xl bg-[#8a9a7b] py-2 text-sm font-bold text-white transition-transform hover:scale-[1.02]">
                          Apply for this Role
                        </button>
                      )}
                    </div>
