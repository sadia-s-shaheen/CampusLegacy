...
export function ChatView({ contactId, contactName }: { contactId: string, contactName: string }) {
  ...
  useEffect(() => {
    const initChat = async () => {
      ...
      if (convId) {
        ...
        // 3. Subscribe to Realtime Updates (Optional but recommended)
  ...
  const handleSend = async (e: React.FormEvent) => {
    ...
    const tempMsg = { ...newMessage, id: Date.now(), created_at: new Date().toISOString() }
  ...
  if (loading) {
    return <div className="flex min-h-dvh items-center justify-center bg-[#e8e9e8]"><Loader2 className="size-8 animate-spin text-[#668184]" /></div>
  }
    ...
    <div className="relative flex min-h-dvh flex-col bg-[#e8e9e8] text-[#22393c]">
      ...
      <main className="relative flex-1 overflow-y-auto px-4 py-6 space-y-4 pb-40">
        {messages.map((msg, index) => {
          const isSent = msg.sender_id !== contactId // Simplified logic
          return (
            <motion.div key={msg.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.02 }} className={`flex ${isSent ? "justify-end" : "justify-start"}`}>
