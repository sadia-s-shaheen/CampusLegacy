...
import { ArrowLeft, Loader2, Users, FolderGit2, Tag, Briefcase, Sparkles, UserPlus, MessageCircle, Check, Info, X } from "lucide-react"
...
export default function DiscoverAllPage() {
  ...
  const type = (searchParams.get("type") as Tab) || "Projects"
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (type === "People") {
          ...
          const { data: d } = await supabase.from("project_roles").select("id, project_id, title, description, slots, projects(id, title)").eq("status", "open")
          data = d || []
        }
        ...
        {loading ? (
          <div className="flex h-[620px] items-center justify-center">
            <div className="flex flex-col items-center gap-3">
              ...
              <p className="text-xs text-[#668184]">Loading...</p>
            </div>
          </div>
        ) : items.length === 0 ? (
          <div className="flex h-[620px] items-center justify-center">
          ...
          <div className="space-y-4">
            ...
            {type === "Roles" && items.map((r: any) => (
              <Link key={r.id} href={`/projects/${r.project_id}`} className="block">
